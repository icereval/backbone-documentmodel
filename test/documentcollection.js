$(document).ready(function() {
    ...
    module("Backbone.DocumentCollection", _.extend(new Environment, {

        setup: function() {
            Environment.prototype.setup.apply(this, arguments);

            a         = new Backbone.DocumentModel({id: 3, label: 'a'});
            b         = new Backbone.DocumentModel({id: 2, label: 'b'});
            c         = new Backbone.DocumentModel({id: 1, label: 'c'});
            d         = new Backbone.DocumentModel({id: 0, label: 'd'});
            e         = null;
            col       = new Backbone.DocumentCollection([a,b,c,d]);
            otherCol  = new Backbone.DocumentCollection();
        }
    ...
    test("String comparator.", 1, function() {
        var collection = new Backbone.DocumentCollection([
            {id: 3},
            {id: 1},
            {id: 2}
        ], {comparator: 'id'});
    ...
    test("get with non-default ids", 5, function() {
        var col = new Backbone.DocumentCollection();
        var MongoModel = Backbone.DocumentModel.extend({idAttribute: '_id'});
        ...
        var col2 = new Backbone.DocumentCollection();
    ...
    test("add", 10, function() {
        ...
        e = new Backbone.DocumentModel({id: 10, label : 'e'});
        otherCol.add(e);
        ...
        var f = new Backbone.DocumentModel({id: 20, label : 'f'});
        var g = new Backbone.DocumentModel({id: 21, label : 'g'});
        var h = new Backbone.DocumentModel({id: 22, label : 'h'});
        var atCol = new Backbone.DocumentCollection([f, g, h]);
    ...
    test("add multiple models", 6, function() {
        var col = new Backbone.DocumentCollection([{at: 0}, {at: 1}, {at: 9}]);
    ...
    test("add; at should have preference over comparator", 1, function() {
        var Col = Backbone.DocumentCollection.extend({
        ...
        col.add(new Backbone.DocumentModel({id: 1}), {at:   1});

        equal(col.pluck('id').join(' '), '3 1 2');
    });

    test("can't add model to collection twice", function() {
        var col = new Backbone.DocumentCollection([{id: 1}, {id: 2}, {id: 1}, {id: 2}, {id: 3}]);
    ...
    test("add model to multiple collections", 10, function() {
        var counter = 0;
        var e = new Backbone.DocumentModel({id: 10, label : 'e'});
        ...
        var colE = new Backbone.DocumentCollection([]);
        colE.on('add', function(model, collection) {
            equal(e, model);
            equal(colE, collection);
        });
        var colF = new Backbone.DocumentCollection([]);
    ...
    test("add model with parse", 1, function() {
        ...
        var Col = Backbone.DocumentCollection.extend({model: Model});
        var col = new Col;
    ...
    test("add model to collection with sort()-style comparator", 3, function() {
        ...
        var tom = new Backbone.DocumentModel({name: 'Tom'});
        var rob = new Backbone.DocumentModel({name: 'Rob'});
        var tim = new Backbone.DocumentModel({name: 'Tim'});
        col.add(tom);
        col.add(rob);
        col.add(tim);
    ...
    test("shift and pop", 2, function() {
        var col = new Backbone.DocumentCollection([{a: 'a'}, {b: 'b'}, {c: 'c'}]);
    ...
    test("events are unbound on remove", 3, function() {
        var counter = 0;
        var dj = new Backbone.DocumentModel();
        var emcees = new Backbone.DocumentCollection([dj]);
    ...
    test("remove in multiple collections", 7, function() {
        ...
        var passed = false;
        var e = new Backbone.DocumentModel(modelData);
        var f = new Backbone.DocumentModel(modelData);
        f.on('remove', function() {
            passed = true;
        });
        var colE = new Backbone.DocumentCollection([e]);
        var colF = new Backbone.DocumentCollection([f]);
        ok(e != f);
        ok(colE.length === 1);
        ok(colF.length === 1);
        colE.remove(e);
    ...
    test("remove same model in multiple collection", 16, function() {
        var counter = 0;
        var e = new Backbone.DocumentModel({id: 5, title: 'Othello'});
        ...
        var colF = new Backbone.DocumentCollection([e]);
    ...
    test("Colllection: non-persisted model destroy removes from all collections", 3, function() {
        var e = new Backbone.DocumentModel({title: 'Othello'});
    ...
    test("create with validate:true enforces validation", 2, function() {
        var ValidatingModel = Backbone.DocumentModel.extend({
            validate: function(attrs) {
                return "fail";
            }
        });
        var ValidatingCollection = Backbone.DocumentCollection.extend({
    ...
    test("where and findWhere", 8, function() {
        var model = new Backbone.DocumentModel({a: 1});
        var coll = new Backbone.DocumentCollection([
            model,
            {a: 1},
            {a: 1, b: 2},
            {a: 2, b: 2},
    ...
    test("sortedIndex", function () {
        var model = new Backbone.DocumentModel({key: 2});
        var collection = new (Backbone.DocumentCollection.extend({
            comparator: 'key'
        }))([model, {key: 1}]);
    ...
    test ("reset with different values", function(){
        var col = new Backbone.DocumentCollection({id: 1});
        col.reset({id: 1, a: 1});
        equal(col.get(1).get('a'), 1);
    });

    test("same references in reset", function() {
        var model = new Backbone.DocumentModel({id: 1});
        var collection = new Backbone.DocumentCollection({id: 1});
    ...
    test("reset passes caller options", 3, function() {
        ...
        var col = new (Backbone.DocumentCollection.extend({ model: Model }))();
    ...
    test("add does not alter arguments", 2, function(){
        var attrs = {};
        var models = [attrs];
        new Backbone.DocumentCollection().add(models);
    ...
    test("Invalid models are discarded with validate:true.", 5, function() {
        ...
        collection.model = Backbone.DocumentModel.extend({
    ...
    test("#964 - collection.get return inconsistent", 2, function() {
        var c = new Backbone.DocumentCollection();
    ...
    test("#1112 - passing options.model sets collection.model", 2, function() {
        var Model = Backbone.DocumentModel.extend({});
        var c = new Backbone.DocumentCollection([{id: 1}], {model: Model});
        ok(c.model === Model);
    ...
    test("null and undefined are invalid ids.", 2, function() {
        var model = new Backbone.DocumentModel({id: 1});
        var collection = new Backbone.DocumentCollection([model]);
        model.set({id: null});
    ...
    test("#1355 - `options` is passed to success callbacks", 2, function(){
        var m = new Backbone.DocumentModel({x:1});
        var col = new Backbone.DocumentCollection();
        var opts = {
    ...
    test("#1655 - groupBy can be used with a string argument.", 3, function() {
        var collection = new Backbone.DocumentCollection([{x: 1}, {x: 2}]);
    ...
    test("#1655 - sortBy can be used with a string argument.", 1, function() {
        var collection = new Backbone.DocumentCollection([{x: 3}, {x: 1}, {x: 2}]);
    ...
    test("`sort` shouldn't always fire on `add`", 1, function() {
        var c = new Backbone.DocumentCollection([{id: 1}, {id: 2}, {id: 3}], {
            comparator: 'id'
        });
    ...
    test("set", function() {
        var m1 = new Backbone.DocumentModel();
        var m2 = new Backbone.DocumentModel({id: 2});
        var m3 = new Backbone.DocumentModel();
        var c = new Backbone.DocumentCollection([m1, m2]);
    ...
    test("set with only cids", 3, function() {
        var m1 = new Backbone.DocumentModel;
        var m2 = new Backbone.DocumentModel;
        var c = new Backbone.DocumentCollection;
        c.set([m1, m2]);
        equal(c.length, 2);
        c.set([m1]);
    ...
    test("set + merge with default values defined", function() {
        ...
        var col = new Backbone.DocumentCollection([m], {model: Model});
    ...
    test("`set` with non-normal id", function() {
        var Collection = Backbone.DocumentCollection.extend({
            model: Backbone.DocumentModel.extend({idAttribute: '_id'})
        });
    ...
    // --- Backbone.DocumentCollection expected functionality tests ---

    test("setting an array", 3, function() {
        var col = new Backbone.DocumentCollection([
            { custom: { foo: 1 } },
            { custom: { foo: 2 } }
        ]);
        ...
        col.set([
            { custom: { foo: 1 } } // set will cause an 'add' to fire.
        ]);
        col.set([
            { custom: { foo: 2 } } // set will cause an 'add' to fire.
        ]);
        equal(col.models.length, 1);
    });

    test("setting an array using deep model dot notation", 2, function() {
        var col = new Backbone.DocumentCollection([]);
        col.on('add', function() {
            ok(1);
        });
        col.on('add:*', function() {
            ok(0); // should not hit as the event is fired on the parent collection.
        });
        col.add([
            { custom: { foo: 1 } }
        ]);
    ...
    test("setting an object to existing model using an object", 4, function() {
        ...
        equal(model.get('custom').get('foo'), 1);
        equal(model.get('custom').get('nested'), undefined);
        ...
        equal(model.get('custom').get('foo'), 2);
        equal(model.get('custom').get('nested').get('foo'), 3);
    });

    test("setting an object to existing model using deep model dot notation", 4, function() {
        var model = new Backbone.DocumentModel({
            'custom.foo': 1
        });

        equal(model.get('custom.foo'), 1);
        equal(model.get('custom.nested.foo'), undefined);
        ...
        equal(model.get('custom.foo'), 2);
        equal(model.get('custom.nested.foo'), 3);
    });

    test("setting an array to existing model using an object", 10, function() {
        var model = new Backbone.DocumentModel({
            items: [
                { foo: 1 }
            ]
        ...
        equal(model.get('items').at(0).get('foo'), 1);
        equal(model.get('items').at(1), undefined);
        ...
        equal(model.get('items').at(0).get('foo'), 2);
        equal(model.get('items').at(1).get('foo'), 3);

        model.get('items').add({ foo: 4 });

        equal(model.get('items').at(2).get('foo'), 4);
        ...
        equal(model.get('items').at(3).get('foo'), 5);
        equal(model.get('items').at(4).get('foo'), 6);
        ...
        equal(model.get('items').at(0).get('foo'), 1);
        equal(model.get('items').at(1), undefined);
        equal(model.get('items').models.length, 1);
    });

    test("setting an array to existing model using deep model dot notation", 10, function() {
        var model = new Backbone.DocumentModel({
            'items.0.foo': 1
        });

        equal(model.get('items.0.foo'), 1);
        equal(model.get('items.1.foo'), undefined);
        ...
        equal(model.get('items.0.foo'), 2);
        equal(model.get('items.1.foo'), 3);

        model.get('items').add({ foo: 4 });

        equal(model.get('items.2.foo'), 4);
        ...
        equal(model.get('items.3.foo'), 5);
        equal(model.get('items.4.foo'), 6);
