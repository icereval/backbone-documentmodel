$(document).ready(function() {
    ...
    var proxy = Backbone.DocumentModel.extend();
    var klass = Backbone.DocumentCollection.extend({
        url : function() { return '/collection'; }
    });
    var doc, collection;

    module("Backbone.DocumentModel", _.extend(new Environment, {
    ...
    test('url and urlRoot are directly attached if passed in the options', 2, function () {
        var model = new Backbone.DocumentModel({a: 1}, {url: '/test'});
        var model2 = new Backbone.DocumentModel({a: 2}, {urlRoot: '/test2'});
        equal(model.url, '/test');
        equal(model2.urlRoot, '/test2');
    });

    test("underscore methods", 5, function() {
        var model = new Backbone.DocumentModel({ 'foo': 'a', 'bar': 'b', 'baz': 'c' });
    ...
    test("clone", 10, function() {
        var a = new Backbone.DocumentModel({ 'foo': 1, 'bar': 2, 'baz': 3});
        var b = a.clone();
        equal(a.get('foo'), 1);
        equal(a.get('bar'), 2);
        equal(a.get('baz'), 3);
        ...
        var foo = new Backbone.DocumentModel({p: 1});
        var bar = new Backbone.DocumentModel({p: 2});
    ...
    test("isNew", 6, function() {
        ...
        a = new Backbone.DocumentModel({ 'foo': 1, 'bar': 2, 'baz': 3, 'id': -5 });
        ok(!a.isNew(), "any defined ID is legal, negative or positive");
        a = new Backbone.DocumentModel({ 'foo': 1, 'bar': 2, 'baz': 3, 'id': 0 });
        ok(!a.isNew(), "any defined ID is legal, including zero");
        ok( new Backbone.DocumentModel({          }).isNew(), "is true when there is no id");
        ok(!new Backbone.DocumentModel({ 'id': 2  }).isNew(), "is false for a positive integer");
        ok(!new Backbone.DocumentModel({ 'id': -5 }).isNew(), "is false for a negative integer");
    });
    ...
    test("set and unset", 8, function() {
        var a = new Backbone.DocumentModel({id: 'id', foo: 1, bar: 2, baz: 3});
        var changeCount = 0;
    ...
    test("set falsy values in the correct order", 2, function() {
        var model = new Backbone.DocumentModel({result: 'result'});
    ...
    test("set an empty string", 1, function() {
        var model = new Backbone.DocumentModel({name : "Model"});
        model.set({name : ''});
        equal(model.get('name'), '');
    });

    // MODIFIED to expected DocumentModel behavior.
    test("setting an object", 3, function() {
        var model = new Backbone.DocumentModel({
            custom: { foo: 1 }
        });
        model.on('change:*', function() {
    ...
    test("clear", 3, function() {
        var changed;
        var model = new Backbone.DocumentModel({id: 1, name : "Model"});
    ...
    test("defaults", 4, function() {
        var Defaulted = Backbone.DocumentModel.extend({
            defaults: {
                "one": 1,
                "two": 2
            }
        ...
        Defaulted = Backbone.DocumentModel.extend({
            defaults: function() {
                return {
                    "one": 3,
                    "two": 4
    ...
    test("change, hasChanged, changedAttributes, previous, previousAttributes", 9, function() {
        var model = new Backbone.DocumentModel({name: "Tim", age: 10});
    ...
    test("changedAttributes", 3, function() {
        var model = new Backbone.DocumentModel({a: 'a', b: 'b'});
        deepEqual(model.changedAttributes(), false);
    ...
    test("change with options", 2, function() {
        var value;
        var model = new Backbone.DocumentModel({name: 'Rob'});
    ...
    test("change after initialize", 1, function () {
        ...
        var obj = new Backbone.DocumentModel(attrs);
    ...
    test("save within change event", 1, function () {
        var env = this;
        var model = new Backbone.DocumentModel({firstName : "Taylor", lastName: "Swift"});
        model.url = '/test';
        model.on('change', function () {
            model.save();
    ...
    test("validate after save", 2, function() {
        var lastError, model = new Backbone.DocumentModel();
    ...
    test("destroy", 3, function() {
        ...
        var newModel = new Backbone.DocumentModel;
    ...
    test("validate on unset and clear", 6, function() {
        var error;
        var model = new Backbone.DocumentModel({name: "One"});
        model.validate = function(attrs) {
            if (!attrs.name) {
                error = true;
                return "No thanks.";
    ...
    test("Inherit class properties", 6, function() {
        var Parent = Backbone.DocumentModel.extend({
    ...
    test("hasChanged/set should use same comparison", 2, function() {
        var changed = 0, model = new Backbone.DocumentModel({a: null});
        model.on('change', function() {
    ...
    test("set same value does not trigger change", 0, function() {
        var model = new Backbone.DocumentModel({x: 1});
    ...
    test("unset does not fire a change for undefined attributes", 0, function() {
        var model = new Backbone.DocumentModel({x: undefined});
    ...
    test("`previous` for falsey keys", 2, function() {
        var model = new Backbone.DocumentModel({0: true, '': true});
    ...
    test("`save` with `wait` sends correct attributes", 5, function() {
        var changed = 0;
        var model = new Backbone.DocumentModel({x: 1, y: 2});
        model.url = '/test';
    ...
    test("isValid", function() {
        var model = new Backbone.DocumentModel({valid: true});
        model.validate = function(attrs) {
    ...
    // --- Backbone.DocumentModel expected functionality tests ---
    ...
    test("setting an array to existing model using deep model dot notation", 12, function() {
        var model = new Backbone.DocumentModel({
            items: [
                { foo: 1 }
            ]
        ...
        equal(model.get('items.0.foo'), 6);
        equal(model.get('items').models.length, 4);
        ...
        equal(model.get('items.0.foo'), 7);
        equal(model.get('items').models.length, 1);
    });

    test("nested model events", 8, function() {
        var model = new Backbone.DocumentModel({
            items: [
                { foo: 1 },
                { foo: 2 }
        ...
        model.on('change', function () {
            ok(1); // model should not receive 'change'.
        });
        ...
        model.on('change:items.foo', function () {
            ok(1);
        });
